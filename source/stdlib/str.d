...
int strcpy(ref Machine machine, double[] p)
{
    ...
    {
        int pos = i + mempos;
        if (pos > machine.currThread.mem.length - 1)
            machine.currThread.mem.length = pos + 1;
...
int strcmp(ref Machine machine, double[] p)
{
    ...
    double register = p[2];
    setRegister(machine, register, readString(machine, cast(int) params[0]) == readString(machine, cast(
            int) params[1]));
    return 3;
}
