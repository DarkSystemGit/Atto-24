...
int newStaticArray(ref Machine machine, double[] p)
{
    ...
    heapObj obj = machine.currThread.heap.getObj(cast(int) arrstruct.length);
    double objaddr = machine.currThread.heap.getDataPtr(obj);
...
int newDynamicArray(ref Machine machine, double[] p)
{
    ...
    arr[0] = 0;
    heapObj arrobj = machine.currThread.heap.getObj(cast(int) arr.length);
    machine.currThread.objs.arrayObjs ~= arrayObj(arrobj.id, machine.currThread.heap.getDataPtr(
            arrobj));
    double bodyptr = machine.currThread.heap.getDataPtr(arrobj);
    arrstruct = [true, capacity, bodyptr];
    heapObj structObj = machine.currThread.heap.getObj(cast(int) arrstruct.length);
    double arrptr = machine.currThread.heap.getDataPtr(structObj);
...
int getArrayLength(ref Machine machine, double[] p)
{
    ...
    {
        length = machine.currThread.mem[cast(
                ulong) machine.currThread.mem[cast(ulong)(params[0] + 2)]];
    }
    setRegister(machine, p[1], length);
    return 2;
}
...
UserArray getArray(ref Machine machine, double addr)
{
    ...
    if (machine.currThread.mem[cast(ulong) addr] == 0)
    {
        ...
        arr.body = machine.currThread.mem[cast(ulong)(addr + 3) .. cast(ulong)(
                addr + 3 + *arr.capacity)];
    ...
    else
    {
        arr.dynamic = true;
        arr.ptr = &machine.currThread.mem[cast(ulong)(addr + 2)];
        arr.body = machine.currThread.mem[cast(ulong)(*arr.ptr + 1) .. cast(
                ulong)(*arr.ptr + 1 + *arr.capacity)];
...
void growArray(ref Machine machine, ref UserArray arr, double newlength)
{
    if ((newlength >= *arr.capacity) && arr.dynamic)
    {
        ...
        utils.write(machine, newObj.start, machine.currThread.mem[cast(ulong)(
                    *arr.ptr) .. cast(ulong)(*arr.ptr + 1 + *arr.capacity)]);
        foreach (heapObj obj; machine.currThread.heap.hobjs)
        {
            if (obj.start == *arr.ptr)
            {
                currObj = obj;
                break;
            }
        ...
        *arr.ptr = cast(double) newObj.start;
        arr.body = machine.currThread.mem[cast(ulong)(*arr.ptr) .. cast(ulong)(
                *arr.ptr + *arr.capacity)];
        *arr.length = oldlength;
        //writeln([*arr.length,*arr.capacity,*arr.ptr],arr.body);
    }
    else if (!arr.dynamic)
        throw new Exception("Cannot grow static array");
}
...
int arrayPush(ref Machine machine, double[] p)
{
    ...
    if (arr.dynamic)
    {
        growArray(machine, arr, *arr.length + 1);
    }
    else if (*arr.length + 1 > *arr.capacity)
    {
        throw new Exception("Exceeded array capcity");
    };
    (*arr.length)++;
...
int arrayPop(ref Machine machine, double[] p)
{
    ...
    double[] newbody = (arr.body).replaceSlice(arr.body[cast(ulong) params[1] .. $], arr.body[cast(
                ulong) params[1] + 1 .. $]);
    (*arr.length)--;
...
int arraySlice(ref Machine machine, double[] p)
{
    ...
    double end = params[2];
    UserArray arr = getArray(machine, params[0]);
    double[] newarrstruct = [false, end - start, end - start] ~ arr.body[cast(
            ulong) start .. cast(ulong) end];
    heapObj obj = machine.currThread.heap.getObj(cast(int) newarrstruct.length);
    double objaddr = machine.currThread.heap.getDataPtr(obj);
...
int arraySplice(ref Machine machine, double[] p)
{
    ...
    utils.write(machine, objaddr, newarrstruct);
    setRegister(machine, p[3], objaddr);
    double[] newbody = arr.body.replaceSlice(arr.body[cast(ulong) start .. $], arr.body[cast(
                ulong) end .. $]);
...
int arrayInsert(ref Machine machine, double[] p)
{
    ...
    *arr.length = *data.length + *arr.length;
    double[] newbody = arr.body.replaceSlice(arr.body[cast(ulong) params[1] .. $], data.body ~ arr.body[cast(
                ulong) params[1] .. cast(ulong)*arr.length]);
    newbody.length = arr.body.length;
    arr.body[0 .. $] = newbody;
    return 3;
}
...
int arrayFreeDynamic(ref Machine machine, double[] p)
{
    ...
    foreach (int i, arrayObj obj; machine.currThread.objs.arrayObjs)
    {
        ...
        {
            machine.currThread.heap.free(obj.id);
        }
        else
        {
            newObjList ~= obj;
